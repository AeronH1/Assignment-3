...
for eta in n: #iterates over n
    ...
    for shuf in r: #iterates over r
        ...
        for algo in ["SLP", "MLP"]: #iterates over the algorithms
            ...
            #-->add your Python code here
            if algo == "SLP":
                clf = Perceptron(eta0 = eta, shuffle = shuf, max_iter = 1000)
            else:
                clf = MLPClassifier(activation = 'logistic', learning_rate_init = eta, hidden_layer_sizes = (25,), shuffle = shuf, max_iter = 1000)
            ...
            if algo == "SLP":
                if accuracy > highest_perceptron_accuracy:
                    ...
                    print(f"Highest Perceptron accuracy so far: {highest_perceptron_accuracy:.4f}, Parameters: learning rate={eta}, shuffle={shuf}")
            else:
                if accuracy > highest_mlp_accuracy:
                    highest_mlp_accuracy = accuracy
                    print(f"Highest MLP accuracy so far: {highest_mlp_accuracy:.4f}, Parameters: learning rate={eta}, shuffle={shuf}")
print("finished")
